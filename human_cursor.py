...
class HumanizeMouseTrajectory:
    ...
    def generate_path(self, from_point, to_point):
        path = self.generate_curve(from_point, to_point)
        return [(int(i[0]), int(i[1])) for i in path]

    def generate_curve(self, from_point, to_point, **kwargs):
        ...
        left_boundary = (
            kwargs.get("left_boundary", min(from_point[0], to_point[0]))
            - offset_boundary_x
        )
        right_boundary = (
            kwargs.get("right_boundary", max(from_point[0], to_point[0]))
            + offset_boundary_x
        )
        down_boundary = (
            kwargs.get("down_boundary", min(from_point[1], to_point[1]))
            - offset_boundary_y
        )
        up_boundary = (
            kwargs.get("up_boundary", max(from_point[1], to_point[1]))
            + offset_boundary_y
        )
        ...
        points = self.generate_points(internalKnots, from_point, to_point)
    ...
    def generate_points(self, knots, from_point, to_point):
        ...
        midPtsCnt = max(
            abs(from_point[0] - to_point[0]),
            abs(from_point[1] - to_point[1]),
            2,
        )
        knots = [from_point] + knots + [to_point]
